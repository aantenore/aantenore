...
| --- | --- | --- |
| [myMoE](https://github.com/aantenore/myMoE) | Helps a coding assistant try bounded tasks on local models in a configured order and accept only results that pass a deterministic check. It also prevents participating agents from claiming the same observed free memory. | Installable alpha (v0.14.0-alpha.1). LocalCascade routing and receipts are contract-tested offline; live model quality and end-to-end token or cost savings are not yet proven. Cooperative leases coordinate participating processes; they do not reserve RAM or VRAM at operating-system level. |
